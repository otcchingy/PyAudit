...
def write_out_new_cashbook_set_from_config(df: pd.DataFrame, config):
    if config.get('output') is not None:
        for group in config['output'].keys():
            pvs = eval(config['output'][group])
            if len(pvs) > 0:
...
def write_excel(filename, data):
    ...
    data.to_excel(writer, sheet_name='book', index=False)
    writer.save()


def get_cashbook_options(config):
    cashbook_df = pd.read_excel(config['book']['path'], config['book']['sheet'])
